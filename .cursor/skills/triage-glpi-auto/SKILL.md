...
**Regla anti-colapso:** cuando la hipótesis sea de tipo "registro maestro anómalo/config distinta" y el punto 1 (enumeración, incluyendo 1-ter) haya identificado 2 o más columnas `EM_*` candidatas sobre la misma tabla, **cada columna es su propia fila** en esta tabla — nunca se resume en una sola fila con un solo resultado. Una hipótesis general de "registro maestro anómalo" solo se marca **Descartada** cuando **todas** las columnas candidatas enumeradas fueron probadas individualmente y ninguna mostró desviación frente a sus hermanos. Si al menos una columna sí muestra desviación, la hipótesis general se marca **Confirmada** — el resultado alineado de una columna no invalida ni reemplaza el resultado desviado de otra.
